"""
goat_funded.py — GOAT Funded BTC/USD Live Bot  [v1.1 — 2026-03-22]
==============================================
...
v1.1 FIXES (2026-03-22) — four surgical corrections matching btc_trader.py:
  1. ETH decoupled from BULL entry: BULL = btc_bull AND ADX > 15 (was: +eth_bull AND ADX > 20)
  2. ADX threshold lowered 20 → 15 (ranging markets naturally suppress ADX)
  3. BEAR now requires BOTH assets bearish (AND not OR) — OR was permanently locking all entries
  4. NEUTRAL gets full signal suite (was BB_LOWER only — BTC spends ~60% of time in NEUTRAL)
  5. Idle guard restructured as independent if-block (previous elif chain made it dead code in V5)
...
ADX_THRESHOLD     = 15           # minimum ADX to confirm BULL regime (lowered from 20 — ranging markets suppress ADX)
...
def compute_signals(df_btc, df_eth, bb_mult):
    ...
    # Fix: ETH decoupled from BULL entry — BTC + ADX sufficient (matches btc_trader.py)
    # Fix: BEAR requires BOTH assets bearish — OR was permanently locking out entries
    if btc_bull and strong:
        regime = 'BULL'
...
class GoatFunded:
    ...
    def boot(self):
        log_event("=" * 62)
        log_event("GOAT FUNDED — BTC/USD $100k | V4.1 Trail | v1.1 | BULL+NEUTRAL full suite")
        log_event("=" * 62)
    ...
    def hourly_tick(self):
        ...
        # ── ENTRY ─────────────────────────────────────────────
            ...
            if ind['regime'] == 'BEAR':
                ...
                # Fix: idle guard restructured as independent check — previous elif
                # chain made this branch unreachable (dead code in V5/goat original)
                if action == 'HOLD' and idle_8h:
                    if ind['sig_ema_pull'] or ind['sig_sma_touch'] or ind['sig_rsi_os']:
                        action = 'BUY'; sig_name = 'IDLE_GUARD'

            else:  # NEUTRAL — Fix: full signal suite (was BB_LOWER only, missing 60% of market time)
                flat_secs = time.time() - self._last_sell_ts if self._last_sell_ts else IDLE_HOURS * 3600 + 1
                idle_8h   = flat_secs > IDLE_HOURS * 3600
                ...
                if ind['sig_bb_lower']:
                    ...
                    action = 'BUY'; sig_name = 'EMA21_CROSS_NEUTRAL'
                elif ind['sig_ema_pull']:
                    action = 'BUY'; sig_name = 'EMA21_PULL_NEUTRAL'
                elif ind['sig_sma_touch']:
                    action = 'BUY'; sig_name = 'SMA20_TOUCH_NEUTRAL'
                elif ind['sig_rsi_os']:
                    action = 'BUY'; sig_name = 'RSI_OVERSOLD_NEUTRAL'
                if action == 'HOLD' and idle_8h:
                    if ind['sig_ema_pull'] or ind['sig_sma_touch'] or ind['sig_rsi_os']:
                        action = 'BUY'; sig_name = 'IDLE_GUARD_NEUTRAL'
