...
MAX_SPREAD_PCT = 0.55          # Explicit spread filter
MIN_SCANNER_SCORE = 8.0

# 2. Regime Gating
REGIME_TOP_N = 15
REGIME_MIN_AVG_CHANGE = 0.5    # Top 15 must average at least +0.5% 24h change

# 3. Explicit Pullback & Recovery Logic
REQUIRED_SETUP = "continuation"
MIN_PULLBACK_PCT = -2.5        
MAX_PULLBACK_PCT = -0.01       
REQUIRE_GREEN_RECOVERY = True  

# 4. Anti-Chase Filters
MAX_RET_3_PCT = 1.5            # Reject if it pumped > 1.5% in last 3 mins
MAX_RET_5_PCT = 2.5            # Reject if it pumped > 2.5% in last 5 mins
...
class KrakenPoCBot:
    ...
    async def debug_heartbeat(self):
        """Background task to monitor event loop health and execution blocks."""
        while True:
            await asyncio.sleep(HEARTBEAT_EVERY_S)
            now = time.time()
            scan_latency = now - self.last_scan
            ...
            log.info(
                ...
                f"Last Scan: {scan_latency:.1f}s ago | "
                f"Last Risk: {risk_latency:.1f}s ago | "
                f"Active Async Tasks: {tasks}"
            )
            
            if scan_latency > (SCAN_EVERY_S * 5):
                log.warning("⚠️ SCAN LOOP HANG DETECTED. Check Kraken API Rate Limits or Network.")
            if risk_latency > (RISK_LOOP_EVERY_S * 10):
                log.warning("⚠️ RISK LOOP HANG DETECTED. Immediate attention required.")
    ...
    async def scan_and_buy(self):
        ...
        for sym, t in tickers.items():
            ...
            scanner_score = (change_pct * 1.5) - (spread_pct * 10) # Penalize high spreads in score
        ...
        # Sort by score for regime check and processing
        ...
        for cand in candidates_to_check:
            ...
            try:
                ...
                recovery_ok = last_close > recent_low_3 and (is_green if REQUIRE_GREEN_RECOVERY else True)
                if not recovery_ok:
                    self.log_reject(sym, "failed_recovery_check")
                    continue
    ...
    async def run(self):
        ...
        log.info("🚀 Kraken Live Bot Started ($80 PoC Mode - Feature Complete)")
        ...
        while True:
            ...
            if now - self.last_scan >= SCAN_EVERY_S:
