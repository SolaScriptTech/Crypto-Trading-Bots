    ...
    async def log_intelligence(self):
        ...
        for symbol in WHITELIST:
            try:
                ...
                # FORCE numeric types. API responses can sometimes be strings.
                df['close'] = pd.to_numeric(df['close'], errors='coerce')
                
                # 3. Native Pandas Math for Bollinger Bands (Bulletproof)
                sma = df['close'].rolling(20).mean()
                std = df['close'].rolling(20).std()
                bbu = sma + (2 * std)
                bbl = sma - (2 * std)
                
                bb_width = (bbu.iloc[-1] - bbl.iloc[-1]) / sma.iloc[-1]
                
                # 4. RSI (Safely extracted regardless of pandas_ta version naming)
                df.ta.rsi(length=14, append=True)
                rsi_cols = [c for c in df.columns if 'RSI' in c]
                rsi = df[rsi_cols[0]].iloc[-1] if rsi_cols else 50.0
