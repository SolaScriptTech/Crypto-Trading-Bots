...
Strategy:
  ...
  BEAR regime    → no entry, force exit if holding
  Idleness guard → after 8h flat in BULL/NEUTRAL, any one soft signal fires

Regime detection:
  BULL    = price > EMA21 AND EMA21 > EMA55 AND ADX > 15
  BEAR    = EMA21 < EMA55 confirmed for BEAR_CONFIRM consecutive bars
            AND price making lower lows over LOWER_LOWS_BARS
  NEUTRAL = everything else

Changelog:
  v1 → baseline (kraken_v5 logic, 0 trades in 4 days live)
       result: 0 trades, -3.67% (all from legacy bootstrap)
  v2 → neutral trail 1.8%→2.8%, bear confirm 2 bars, ATR filter
       result: -3.67% → +1.99%, sharpe -1.606 → +1.029
  v3 → stronger BEAR filter (lower lows required), EMA55 trend guard
       on NEUTRAL entries. Everything from v2 preserved.
  v4 → two fixes targeting chained losses during selloffs:
    1. Minimum RSI entry filter: skip ALL entries (BULL + NEUTRAL) when
       RSI < MIN_ENTRY_RSI (35). RSI below 35 means panic/momentum selling
       is active — entries here have no bounce energy behind them.
    2. Loss cooldown: after a SELL_TRAIL or SELL_STOP loss, block new
       entries for LOSS_COOLDOWN_BARS (24) bars. Prevents the bot from
       re-entering a falling market repeatedly during a sustained downtrend.
       Cooldown does NOT apply after SELL_TARGET wins — only losses.
    Everything from v3 is preserved unchanged.
...
LOWER_LOWS_BARS = 6        # v3: bars to check for lower lows in BEAR confirmation
NEUTRAL_TREND_GUARD_PCT = 0.015  # v3: max distance below EMA55 allowed for NEUTRAL entry
MIN_ENTRY_RSI  = 35              # v4: skip entries when RSI below this — panic selling active
LOSS_COOLDOWN_BARS = 24          # v4: bars to wait after a trail/stop loss before re-entering
BB_MULT_BULL   = 1.5
BB_MULT_OTHER  = 2.0
ADX_PERIOD     = 14
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # v3: lower lows flag — True if current low < lowest low of prior N bars
    df["lower_low"] = df["l"] < df["l"].shift(1).rolling(LOWER_LOWS_BARS).min()
...
def detect_regime(df: pd.DataFrame, idx: int) -> str:
    ...
    price      = df["c"].iloc[idx]
    adx        = df["adx"].iloc[idx]
    lower_low  = df["lower_low"].iloc[idx]

    btc_bull = (price > ema21) and (ema21 > ema55)
    # v3: BEAR requires EMA crossover AND price making lower lows
    # prevents brief pullbacks in ranging markets from triggering BEAR
    btc_bear = (ema21 < ema55) and (lower_low is True or lower_low == True)
...
def entry_signal(df: pd.DataFrame, idx: int, regime: str,
    ...
    # v3 FIX 2: EMA55 trend guard for NEUTRAL entries
    # If price is more than NEUTRAL_TREND_GUARD_PCT below EMA55 we are in
    # genuine downtrend territory, not a range bounce — skip entry
    if regime == "NEUTRAL" and not pd.isna(ema55):
        if price < ema55 * (1 - NEUTRAL_TREND_GUARD_PCT):
            return False, ""

    # v4 FIX 1: minimum RSI entry filter
    # RSI below MIN_ENTRY_RSI means panic/momentum selling is active.
    # No bounce energy = trail stop catches us before the move develops.
...
class Backtester:
    def __init__(self, starting_capital: float = 2000.0, slippage: float = SLIPPAGE):
        ...
        self.last_loss_idx  = -999 # v4: bar index of last trail/stop loss
    ...
    def run(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(WARMUP, len(df)):
            ...
            # ── ENTRY ─────────────────────────────────────────────────────
                ...
                # v4: don't enter within LOSS_COOLDOWN_BARS of a trail/stop loss
                in_cooldown = (i - self.last_loss_idx) < LOSS_COOLDOWN_BARS
                if not bear_confirmed and not in_cooldown:
    ...
    def _sell(self, df, idx, regime, reason):
        ...
        # v4: record loss bar for cooldown — only on trail/stop losses, not wins
        if reason in ("SELL_TRAIL", "SELL_STOP", "KILL_SWITCH"):
            self.last_loss_idx = idx
...
def main():
    ...
    log("=" * 62)
    log(f"BTC_TRADER BACKTESTER v4 | days={args.days} | "
        f"capital=${args.capital:,.2f} | slippage={args.slippage*100:.1f}bps | "
        f"neutral_trail=2.8% | bear_confirm={BEAR_CONFIRM}bars | "
        f"lower_lows={LOWER_LOWS_BARS}bars | ema55_guard={NEUTRAL_TREND_GUARD_PCT*100:.1f}% | "
        f"min_rsi={MIN_ENTRY_RSI} | loss_cooldown={LOSS_COOLDOWN_BARS}bars | atr_filter=ON")
    log("=" * 62)
