"""
Kraken Autonomous Trading Bot v3
...
# ─────────────────────────────────────────────────────────────
# LOAD .env FILE  (must be in same folder as bot.py)
# ─────────────────────────────────────────────────────────────
load_dotenv()
...
MAX_TRADE_PCT            = 0.45    # Max 45% of balance per trade
MIN_TRADE_USD            = 5.0
...
DRAWDOWN_HALT_PCT        = 0.30    # Halt if 30% of starting capital lost
...
# Active hours PST — bot only OPENS positions during these windows
# Manages/exits positions 24/7 regardless
ACTIVE_HOURS_PST = [
    (1,  4),   # London open
...
class KrakenAPI:
    ...
    def __init__(self, key: str, secret: str):
        self.key = key
        # Pad secret to multiple of 4 before base64 decoding
        secret = secret.strip()
        ...
        try:
            ...
            raise ValueError(
                f"Could not decode API secret. Make sure you copied it exactly from Kraken.\n"
                f"Error: {e}"
            )
...
class SignalEngine:
    ...
    def score(self, candles: list) -> tuple:
        ...
        # 7 ── ATR volatility filter (dampener only)
...
class TradingBot:
    ...
    def _validate_keys(self):
        if not API_KEY or not API_SECRET:
            ...
            log.error("    Create a .env file in the same folder as bot.py with:")
            log.error("      KRAKEN_API_KEY=your_key_here")
            log.error("      KRAKEN_API_SECRET=your_secret_here")
            sys.exit(1)
    ...
    def check_drawdown(self) -> bool:
        ...
        if lost_pct >= DRAWDOWN_HALT_PCT:
            log.warning(
                ...
                f"Balance=${usd:.2f}. Trading paused."
            )
            return False
        return True
    ...
    def run(self):
        log.info("🚀 Kraken Trading Bot v3 starting")
        ...
        try:
            ...
            log.error("   Check your .env file — ensure 'Query Funds' and 'Create Orders' are enabled on Kraken.")
            return
