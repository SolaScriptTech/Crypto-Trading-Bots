...
├── RiskEngine_quant_trader.py  # Risk Management Engine — exit ladder
├── quant_trader.py         # Main orchestrator — boot, loop, orders, state
├── QUANT_TRADER_ARCHITECTURE.md  # This document
├── quant_trader_state.json      # Atomic state file
├── quant_trader_state.json.tmp  # Atomic write buffer
├── quant_trader_audit.csv       # Every trade with full metadata
├── quant_trader_events.log      # Every decision, timestamped
└── EMERGENCY_STOP               # Create this file to halt the bot
```
