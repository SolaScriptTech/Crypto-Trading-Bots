...
# Entry logic timeframes
TIMEFRAME_SWEEP = "15m"          # liquidity sweep + breakout detection timeframe
TIMEFRAME_MOMENTUM = "1m"        # used only for early red candle logic

# Higher timeframes for trend gating
...
# Liquidity sweep breakout parameters
SWEEP_BACKCANDLES = 40
SWEEP_PIVOT_WINDOW = 7

EMA_LEN = 150
EMA_TREND_BACKCANDLES = 15
...
def _ema_trend_signal(df: pd.DataFrame, ema_len: int = EMA_LEN, backcandles: int = EMA_TREND_BACKCANDLES) -> int:
    """
    2 = strong uptrend, 1 = strong downtrend, 0 = mixed
    Strict rule: last N candles all fully above or fully below EMA.
    """
    if df is None or len(df) < max(ema_len, backcandles) + 5:
        return 0
    ...
    above = (np.minimum(opens, closes) > ema).astype(int)
    below = (np.maximum(opens, closes) < ema).astype(int)

    if int(above.iloc[-win:].sum()) == win:
        return 2
    if int(below.iloc[-win:].sum()) == win:
        return 1
    return 0


def _causal_pivots(highs: np.ndarray, lows: np.ndarray, window: int) -> np.ndarray:
    """
    Causal pivot confirmation to avoid lookahead.
    A pivot at i is confirmed at t = i + window, so at time t we can only use pivots <= t - window.
    ...
    for t in range(2 * window, n):
        ...
        hwin = highs[left : right + 1]
        lwin = lows[left : right + 1]
...
def _liquidity_sweep_breakout_bullish(
    df: pd.DataFrame,
    backcandles: int = SWEEP_BACKCANDLES,
    pivot_window: int = SWEEP_PIVOT_WINDOW,
) -> Tuple[bool, float]:
    """
    Bullish sweep + breakout on the most recent candle.

    Conditions:
      - Use only pivots that are confirmed (no future lookahead)
      - Find the most recent confirmed pivot high (PH) within lookback
      - Current close crosses above PH high (first break)
      - There is at least one confirmed pivot low before PH (PL1) and after PH (PL2)
      - Sweep condition: min low of pivot lows after PH < last pivot low before PH

    Returns:
      (ok, breakout_level)
    """
    if df is None or len(df) < (2 * pivot_window + backcandles + 10):
        return False, 0.0

    n = len(df)
    t = n - 1

    start = max(0, t - backcandles)
    usable_end = t - pivot_window
    if usable_end <= start:
        return False, 0.0
    ...
    piv = _causal_pivots(highs, lows, pivot_window)

    hist_idx = np.arange(start, usable_end + 1)
    piv_mask = piv[hist_idx] != 0
    if not np.any(piv_mask):
        return False, 0.0

    piv_idx = hist_idx[piv_mask]

    ph_idx = piv_idx[piv[piv_idx] == 1]
    if len(ph_idx) == 0:
        return False, 0.0

    ph_i = int(ph_idx[-1])
    level = float(highs[ph_i])

    close_t = float(close[t])
    close_prev = float(close[t - 1])

    if not (close_t > level and close_prev <= level):
        return False, level

    pl_before = piv_idx[(piv[piv_idx] == 2) & (piv_idx < ph_i)]
    pl_after = piv_idx[(piv[piv_idx] == 2) & (piv_idx > ph_i)]

    if len(pl_before) == 0 or len(pl_after) == 0:
        return False, level

    pl1 = float(lows[int(pl_before[-1])])
    pl2 = float(np.min(lows[pl_after]))
...
class ApexBot:
    ...
    async def _analyze_symbol(self, symbol: str) -> Optional[Tuple[str, float, float]]:
        async with self._deep_sem:
            try:
                ...
                if not await self._daily_change_ok(symbol):
                    return None

                df_sweep = await self._fetch_ohlcv_df(symbol, TIMEFRAME_SWEEP, 260)
                if df_sweep is None:
                    return None

                ema_sig = _ema_trend_signal(df_sweep, ema_len=EMA_LEN, backcandles=EMA_TREND_BACKCANDLES)
                if ema_sig != 2:
                    return None

                ok, level = _liquidity_sweep_breakout_bullish(
                    df_sweep,
                    backcandles=SWEEP_BACKCANDLES,
                    pivot_window=SWEEP_PIVOT_WINDOW,
                )
                if not ok:
                    return None

                price = float(df_sweep["close"].iloc[-1])

                df1h = await self._fetch_ohlcv_df(symbol, TIMEFRAME_TREND_1H, 220)
                df4h = await self._fetch_ohlcv_df(symbol, TIMEFRAME_TREND_4H, 220)
    ...
    async def run(self) -> None:
        ...
        logging.info("Strategy: stage A tickers filter + stage B 1h and 4h trend gate + liquidity sweep breakout entry on 15m")
        await self.load_markets()
