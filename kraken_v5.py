...
V5 changes vs V4.2:
  ENTRY — Three new BULL entry modes added (additive, do not remove BB entry):
    1. EMA21 Pullback: price dips to 0–0.75% below EMA21 + RSI(14) < 52
       Captures trend-reload dips in uptrends that never touch the lower BB band.
    2. SMA20 Touch: price crosses below SMA20 + RSI < 50
       Catches deeper pullbacks to the mean without requiring a lower-band touch.
    3. RSI Oversold + Trend: RSI(14) < 42 + price above EMA55 (uptrend intact)
       Enters on momentum exhaustion while confirming the broader trend is BULL.
  IDLENESS GUARD — if bot has been FLAT > 8h in BULL regime, requires only ONE
    of the above three softer conditions instead of all three simultaneously.
  STATE FILE FIX — all file paths resolved relative to BASE_DIR (script directory)
    so os.replace() never fails with 'No such file or directory' on .tmp files.
  ORDER BOOK — wall authenticity threshold lowered from 3 cycles to 2 cycles,
    reducing persistent WARN noise from transient new walls.
...
from order_book_v5 import OrderBookEngine
from trail_engine_v5 import TrailEngine
...
STATE_FILE   = os.path.join(BASE_DIR, 'kraken_v5_state.json')
LOG_FILE     = os.path.join(BASE_DIR, 'kraken_auditable_shadow_bot_v5_audit_trail.csv')
EVENT_FILE   = os.path.join(BASE_DIR, 'kraken_v5_events.log')
LEGACY_CSVS  = [
...
class Conductor:
    ...
    # ── boot sequence ─────────────────────────────────────────
    def boot(self):
        log_event("--- KRAKEN V5: EMA-PULLBACK | SMA-TOUCH | RSI-OVERSOLD | "
                  "IDLENESS GUARD | BASEDIR STATE FIX | ANALOG TRAIL | ORDER BOOK GATE ---")
    ...
    def hourly_signal(self):
        ...
        if s.virtual_btc == 0:
            if self.state.current_regime == 'BEAR':
                ...
                if not np.isnan(lower) and last < lower:
                    action = 'BUY'
    ...
    def print_health(self, eq, dd, book_snap):
        ...
        print("\n" + "="*62)
        print(f" KRAKEN V5 HEALTH CHECK | "
