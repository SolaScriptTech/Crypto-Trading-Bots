"""
order_book_v5.py — OrderBookEngine
...
V5 change: wall authenticity threshold lowered from 3 cycles to 2 cycles
to reduce persistent WARN noise from genuinely stable walls.
"""
...
class OrderBookEngine:
    ...
    def fetch(self):
        ...
        # Use keyword args to avoid positional misinterpretation across ccxt versions
        trades = self.rl.call(self.exchange.fetch_trades, self.symbol,
    ...
    def _check_walls(self, book, mid):
        ...
        bid_walls = [(lvl[0], lvl[1]) for lvl in book['bids'] if lvl[0] >= mid - band]
        if bid_walls:
            best_bid = max(bid_walls, key=lambda x: x[1])
            current_walls[round(best_bid[0], 0)] = ('bid', best_bid[1])

        # Find largest ask wall within band
        ask_walls = [(lvl[0], lvl[1]) for lvl in book['asks'] if lvl[0] <= mid + band]
        if ask_walls:
        ...
        for price, (side, size, cycles) in self.wall_history.items():
            ...
            if cycles < 2 and dist_pct < 0.5:
                ...
                auth = 'AUTHENTIC' if cycles >= 2 else f'new ({cycles} cycles)'
                details.append(
                    f"{'Bid' if side=='bid' else 'Ask'} wall @"
                    f"${price:,.0f}: {auth}"
                )
    ...
    def _check_liquidity(self, book, mid):
        ...
        depth = (sum(lvl[1] for lvl in book['bids'] if lvl[0] >= mid - band) +
                 sum(lvl[1] for lvl in book['asks'] if lvl[0] <= mid + band))
